...
describe('uiReducer.js reducer tests', () => {
  ...
  it('Verifies that user state changes when LOGIN_SUCCESS passed', () => {
    const action = { type: 'LOGIN_SUCCESS', user: { name: 'test' } };
    const reducer = uiReducer(action, initialState);
    expect(reducer.get('user')).toEqual({ name: 'test' });
  });

  it('Verifies that user state changes when LOGOUT passed', () => {
